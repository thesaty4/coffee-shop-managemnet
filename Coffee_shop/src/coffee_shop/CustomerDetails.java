...
public class CustomerDetails extends JFrame implements ActionListener{
	...
	private void initialize() {
		...
		Mobile.setBounds(142, 128, 139, 20);
		panel.add(Mobile);
		
		JLabel lblMobile = new JLabel("Mobile");
		lblMobile.setBounds(42, 131, 74, 14);
		panel.add(lblMobile);
		
		JLabel lblTotalPay = new JLabel("Total Pay");
		lblTotalPay.setBounds(42, 159, 114, 14);
		panel.add(lblTotalPay);
		
		TotalPay = new JTextField();
		TotalPay.setColumns(10);
		TotalPay.setBounds(142, 156, 139, 20);
		panel.add(TotalPay);
		
		Update = new JButton("Update");
		Update.setBounds(25, 239, 89, 23);
		panel.add(Update);
		Update.addActionListener(this);
		
		btnDelete = new JButton("Delete");
		btnDelete.setBounds(126, 239, 80, 23);
		panel.add(btnDelete);
		btnDelete.addActionListener(this);
		
		btnCancel = new JButton("Cancel");
		btnCancel.setBounds(216, 239, 89, 23);
		panel.add(btnCancel);
		btnCancel.addActionListener(this);
		
		PurcheaseDate = new JTextField();
		PurcheaseDate.setColumns(10);
		PurcheaseDate.setBounds(142, 184, 139, 20);
		panel.add(PurcheaseDate);
		
		JLabel label = new JLabel("Purchease Date");
		label.setBounds(42, 187, 114, 14);
		panel.add(label);
		...
		try {
			...
			int i=0,j=0;
			String x[]= {"Customer ID","Name","Gender","Mobile","Purchase Date"};
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==btnCancel) {
			this.hide();
		}else if(name.isEmpty()||gender.isEmpty()||mobile.isEmpty()||totalPay.isEmpty()||custId.isEmpty()) {
			...
			if(e.getSource()==btnDelete) {
				...
				if(del==0) {
					...
					String sql2="delete from `bill` where `customer_id`="+cId;
					try {
				...
				if(del==0) {
					...
					String sql4="update `customer_info` set `total_pay`="+TotPay+" where `customer_id`="+cId;
					try {
